...
项目正在使用 `Tauri + React + TypeScript + Python` 构建新版桌面工作台，
目录为 [`desktop_v2`](desktop_v2)。新版保留原有 Python 检测、MiMo
语音识别和 LLM 评分算法，界面通过结构化命令与实时事件流显示：
...
构建 Windows 安装包前需要 Rust、Microsoft C++ Build Tools、WebView2、
Node.js 和 PyInstaller。双击 `构建课析V2.bat` 会先打包 Python 处理
引擎，再生成 Tauri NSIS 安装包。
...
2. 默认同时处理3页；
3. 页面超过90秒时自动分段识别，再按原顺序合并为该页文字；
4. 请求结束或发生错误后立即删除临时音频；
5. “一键完整处理”中，每确认一页的时间与高清截图，就立即提交该页
   语音识别；该页文字返回后立即提交LLM评分；
6. ASR和LLM共用一个云端并发上限，避免两组请求叠加造成服务限流；
7. 保持原有 `transcript.json` 结构，后续工具无需改变。

为保证动画合并准确，程序仍会先完成全视频低分辨率时间线分析。流水线
从逐页精修换页时间和提取高清截图阶段开始；因此不会拿尚未确认的候选页
提前上传，也不会牺牲当前页面识别精度。
...
- 讲话页面覆盖率；
- 简洁模式：每页总分和评分理由；
- 详细证据模式：额外输出PPT关键点、讲话关键点、对应证据和无关内容。

简洁模式下，模型仍返回三个分项数字供程序稳定计算总分，但不再生成
关键点和对应证据列表，从而减少输出Token。
